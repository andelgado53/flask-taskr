...
def tasks():
	...
	g.db = connect_db()
	cur = g.db.execute(
		'select name, due_date, priority, task_id from task where status=1'
		)
	open_tasks = [ dict(name=row[0], due_date=row[1], priority=row[2], task_id=row[3])
		for row in cur.fetchall()
		]
	cur = g.db.execute(
		'select name, due_date, priority, task_id from task where status=0')
